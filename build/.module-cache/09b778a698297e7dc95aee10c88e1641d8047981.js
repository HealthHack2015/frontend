	...
	render: function() {
		...
		return (
        	React.createElement("div", {className: "diseaseSelector col-md-4"}, 
                React.createElement("input", {className: this.props.name, type: "text", value: this.state.searchString, onBlur: this.handleOutFocus, onChange: this.handleOnChange, placeholder: "Type here"}), 
...
var Dashboard = React.createClass({displayName: "Dashboard",
	...
	handleOnClick: function(new_input, index) {
		...
		this.state.setState({selectors: curr_selectors});
	},

	render: function() {
		var selector_objects = this.state.selectors.map(function(selector, index) {
			return (
				React.createElement(Selector, {key: index, index: index, selected: selector.selected, input: selector.input, handleOnClick: this.handleOnClick, diseases: this.props.diseases})
			);
		})

		var add_new;
		if (this.state.selectors.length < 3) {
			add_new = (React.createElement("button", {type: "button", className: "btn btn-default", onClick: this.addNewSelector}, "Add New Disease"))
		}
...
$.ajax({
	...
	success: function(data) {
		console.log(data);
		React.render(
			React.createElement(Dashboard, {diseases: data}),
